...
void macro_accettanza(){
  ...
  TH1D* h_dist = new TH1D("dist","dist",125,1,3.5);
  ...
  double d = 1;  // 12; //distanza tra i due scintillatori (cm)
  ...
  for(int i=0; i<1e6; i++){
    ...
    double dist = sqrt( pow((x-x_down),2) + pow((y-y_down),2) + pow(d,2) );   
    ...
    if( x_down > 0  && y_down > 0  && x_down < 20.  && y_down < 122. ){
      ...
      h_dist->Fill(dist);
      h_dist->SetBinError(i, h_dist->GetBinError(i));
